(ns resale.operation
  "OperationActor — one intake/authenticate/sale/disclosure/correction
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (ResaleAdvisor-LLM) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the ResaleGovernor
  ...
  Human-in-the-loop: `interrupt-before #{:request-approval}` pauses the
  actor and hands the decision to a human reviewer (compliance officer).
  The reviewer resumes with `{:approval {:status :approved}}` (or
  `:rejected`)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [resale.llm :as llm]
            [resale.policy :as policy]
            [resale.phase :as phase]
...
(defn build
  ...
  `resale.store/Store`).
  opts:
    :advisor      — a `resale.llm/Advisor` (default: mock-advisor)
               ...
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:correction? verdict) :dispute-request
                                          (:high-value? verdict) :high-value-holding-period
                                          :else :low-confidence))
                        :phase ph
